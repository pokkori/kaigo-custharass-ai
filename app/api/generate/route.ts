...
2. **リスクの根拠を法的根拠で示す**
   ...
   - 判例や行政通知がある場合は引用する（不確かな場合は「関連通知として」と明記）
...
6. **エスカレーション判定を必ず含める**
   出力の最後に以下を明示すること:
   - 【事業所内対応で解決可能か】: 可能 / 困難 の判定と理由
   - 【外部連携が必要な閾値】: 「以下の状況になったら国保連・市区町村・警察・弁護士に連携してください」
   - 【今すぐやるべき証拠保全アクション】: チェックボックス形式で3〜5項目（今日中にできること）
     例: □ 発言の逐語録を時刻付きで記録する / □ 録音データをクラウドへバックアップ / □ 目撃者スタッフの証言をメモ

7. **カスハラと正当な苦情を必ず区別する**
   - 「この行為はカスハラに該当します」「これは正当なご意見です」を明示する
   - 厚労省ハラスメント対策指針の定義に基づき判定する`;
const FREE_LIMIT = 3;
